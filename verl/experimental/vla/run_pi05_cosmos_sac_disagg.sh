...
export RAY_ADDRESS=${RAY_ADDRESS:?Set RAY_ADDRESS to the train-rollout head node, for example 10.0.0.1:6379}
...
OUTPUT_DIR=${MLP_MODEL_OUTPUT:-"$HOME/models/vla_cosmos_sac_disagg"}
VIDEO_OUTPUT=${VIDEO_OUTPUT:-"$HOME/video/cosmos_sac_disagg"}
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-"cosmos_sac_disagg"}
...
python -m verl.experimental.vla.main_sac \
    ...
    env.disagg_sim.nnodes=$NUM_ENV_NODES \
